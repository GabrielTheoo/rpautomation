...
import { Plus, Trash2, Save, Info, Search, ChevronUp, ChevronDown } from "lucide-react";
...
export default function TierConfig() {
  ...
  const [newEntry, setNewEntry] = useState<{keyword:string;name:string;tier:1|2|3}>({keyword:"",name:"",tier:1});
  ...
  const handleAdd = () => {
    ...
    if (entries.some((e) => e.keyword.toLowerCase()===newEntry.keyword.trim().toLowerCase())) { setAddError("Já existe uma entrada com essa keyword."); return; }
    ...
    setEntries(updated); saveToStorage(updated);
    setNewEntry({keyword:"",name:"",tier:1});
  };

  const handleDelete = (id: string) => { const updated = entries.filter((e)=>e.id!==id); setEntries(updated); saveToStorage(updated); };
  const handleTierChange = (id: string, tier: 1|2|3) => { const updated = entries.map((e)=>e.id===id?{...e,tier}:e); setEntries(updated); saveToStorage(updated); };
  const toggleSort = (field: "tier"|"name"|"keyword") => { if(sortField===field) setSortDir((d)=>d==="asc"?"desc":"asc"); else { setSortField(field); setSortDir("asc"); } };
          ...
          <div className="bg-card border border-border rounded-2xl p-6 mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
              <div>
                ...
                <input type="text" placeholder="ex: Panorama Farmacêutico" value={newEntry.name} onChange={(e)=>setNewEntry((n)=>({...n,name:e.target.value}))} onKeyDown={(e)=>e.key==="Enter"&&handleAdd()} className="w-full bg-bg border border-border rounded-xl px-3 py-2.5 text-text-base text-sm focus:outline-none focus:border-primary placeholder-text-muted/50 transition-colors" />
              </div>
              <div>
                ...
                <input type="text" placeholder="ex: panoramafarmaceutico" value={newEntry.keyword} onChange={(e)=>setNewEntry((n)=>({...n,keyword:e.target.value}))} onKeyDown={(e)=>e.key==="Enter"&&handleAdd()} className="w-full bg-bg border border-border rounded-xl px-3 py-2.5 text-text-base text-sm focus:outline-none focus:border-primary placeholder-text-muted/50 transition-colors font-mono" />
              </div>
              <div>
                <label className="block text-text-muted text-xs mb-1.5">Tier</label>
                <select value={newEntry.tier} onChange={(e)=>setNewEntry((n)=>({...n,tier:Number(e.target.value) as 1|2|3}))} className="w-full bg-bg border border-border rounded-xl px-3 py-2.5 text-text-base text-sm focus:outline-none focus:border-primary transition-colors">
            ...
            <button onClick={handleAdd} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-bg font-semibold text-sm hover:bg-primary-dark transition-all"><Plus className="w-4 h-4" />Adicionar</button>
            {saved && <span className="ml-3 text-accent text-sm"><Save className="w-3.5 h-3.5 inline" /> Salvo!</span>}
          </div>

          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
              <Search className="w-4 h-4 text-text-muted flex-shrink-0" />
              <input type="text" placeholder="Buscar por nome ou keyword…" value={search} onChange={(e)=>setSearch(e.target.value)} className="flex-1 bg-transparent text-text-base text-sm focus:outline-none placeholder-text-muted/50" />
            ...
            {entries.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-text-muted text-sm mt-1">Adicione veículos acima para classificação automática</p>
              </div>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left px-4 py-3 text-text-muted text-xs font-semibold"><button onClick={()=>toggleSort("name")} className="flex items-center gap-1 hover:text-text-base transition-colors">Nome<SortIcon field="name" /></button></th>
                    <th className="text-left px-4 py-3 text-text-muted text-xs font-semibold"><button onClick={()=>toggleSort("keyword")} className="flex items-center gap-1 hover:text-text-base transition-colors">Keyword no URL<SortIcon field="keyword" /></button></th>
                    <th className="text-left px-4 py-3 text-text-muted text-xs font-semibold"><button onClick={()=>toggleSort("tier")} className="flex items-center gap-1 hover:text-text-base transition-colors">Tier<SortIcon field="tier" /></button></th>
                    ...
                    return (
                      <tr key={entry.id} className="border-b border-border hover:bg-card-hover transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <select value={entry.tier} onChange={(e)=>handleTierChange(entry.id,Number(e.target.value) as 1|2|3)} className={`text-xs font-semibold px-2 py-1 rounded-lg border cursor-pointer focus:outline-none transition-colors ${cfg.bg} ${cfg.border} ${cfg.text}`}>
                            <option value={1}>Tier 1</option><option value={2}>Tier 2</option><option value={3}>Tier 3</option>
                          </select>
                        </td>
                        <td className="px-4 py-3 text-right"><button onClick={()=>handleDelete(entry.id)} className="p-1.5 rounded-lg text-text-muted hover:text-red-400 hover:bg-red-500/10 transition-all" title="Remover"><Trash2 className="w-3.5 h-3.5" /></button></td>
                      </tr>
                    );
                  })}
                </tbody>
