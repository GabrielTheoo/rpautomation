...
import {
  ArrowLeft, Download, Loader2, Zap, TrendingUp,
  Globe, FileText, BarChart3, CheckCircle2,
} from "lucide-react";
...
function ReviewDashboard({ rows, allRows, onExport, exporting }: {
  rows: ProcessedRow[];
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-4 flex-wrap sm:flex-nowrap">
        <KpiCard
          ...
          value={n}
          sub="matérias no período"
          footer={`${uniqSrc} veículos únicos`}
          color="primary"
        />
        <KpiCard
          label="AVE Total"
          value={aveFmt.display}
          sub="valor equivalente em mídia"
          footer={`Tier 1: ${aveTier1Fmt.display}`}
          color="accent"
        />
        <KpiCard
          label="Alcance Total"
          value={reachDisplay}
          sub="leitores potenciais"
          footer={`Tier 1: ${reachT1Display}`}
          color="accent"
        />
        <KpiCard
          label="% Positividade"
          value={`${positividade}%`}
          sub="saldo positivo geral"
          bar={Math.max(0, Math.min(100, positividade))}
          color="primary"
        />
      </div>
      ...
      <div className="flex gap-4 flex-wrap sm:flex-nowrap">
        <MetricCard
          ...
          sub={`Reach T1: ${reachT1Display}`}
          badge={`${tier1Pos} T1 positivas`}
          color="accent"
        />
        <MetricCard
          label="Com Impacto (T1)"
          value={impactTier1}
          sub={`de ${tier1.length} matérias Tier 1`}
          barPct={tier1.length > 0 ? Math.round((impactTier1 / tier1.length) * 100) : 0}
          color="primary"
        />
        <MetricCard
          label="Proativas c/ Impacto"
          value={proativeWithImpact}
          sub={`AVE: ${aveProativeFmt.display}`}
          barPct={n > 0 ? Math.round((proativeWithImpact / n) * 100) : 0}
          color="accent"
        />
        <MetricCard
          label="Tier 1 (todos países)"
          value={tier1AllCountries}
          sub="inclui internacionais"
          badge={tier1AllCountries > tier1.length ? `+${tier1AllCountries - tier1.length} internacionais` : undefined}
          color="primary"
        />
        <MetricCard
          label="AVE c/ Impacto (geral)"
          value={aveWithImpactFmt.display}
          sub="todos os tiers"
          barPct={n > 0 ? Math.round((withImpact / n) * 100) : 0}
          color="accent"
        />
      </div>
    </div>
...
// ─── Page ────────────────────────────────────────────
export default function HistoricoReview() {
  ...
  // Unique countries from entry rows
        ...
        <div className="border-b border-border bg-card shadow-sm">
          <div className="w-full px-4 sm:px-6 py-3 flex flex-col gap-0.5">
            ...
            <span className="text-xs text-text-muted hidden sm:block">
              Histórico → {entry.fileName} → Review
            </span>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div className="flex flex-col gap-0.5">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-text-base">Review — Resumo & Gráficos</h2>
              </div>
